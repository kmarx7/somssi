...
import { createConfirmedBooking, BookingServiceError } from "@/lib/mock-bookings";
...
export async function submitBooking(_prev: BookingActionState, formData: FormData): Promise<BookingActionState> {
  ...
  let booking;
  try { booking = await createConfirmedBooking({ experienceSlug, sessionId, guestCount, optionCodes, firstName, lastName, email, country, phone: get("phone") || undefined, language }); }
